...
public class AsyncHttpClient {
    ...
    private static SchemeRegistry getDefaultSchemeRegistry(boolean fixNoHttpResponseException, int httpPort, int httpsPort) {
        ...
        if (httpPort < 1) {
            httpPort = 80;
			EsLog.d(LOG_TAG, "Invalid HTTP port number specified, defaulting to 80");
        }

        if (httpsPort < 1) {
            httpsPort = 443;
			EsLog.d(LOG_TAG, "Invalid HTTPS port number specified, defaulting to 443");
        }
    ...
    public void setCredentials(AuthScope authScope, Credentials credentials) {
        if (credentials == null) {
			EsLog.d(LOG_TAG, "Provided credentials are null, not setting");
            return;
        }
    ...
    public void cancelRequests(final Context context, final boolean mayInterruptIfRunning) {
        if (context == null) {
			EsLog.e(LOG_TAG, "Passed null Context to cancelRequests");
            return;
        }
    ...
    protected RequestHandle sendRequest(DefaultHttpClient client, HttpContext httpContext, HttpUriRequest uriRequest, String contentType, ResponseHandlerInterface responseHandler, Context context) {
        ...
        if (context != null) {
			...
			// responseHandler).updateRequestHeaders(uriRequest);
    ...
    public static void silentCloseInputStream(InputStream is) {
        try {
			...
			EsLog.w(LOG_TAG, "Cannot close input stream", e);
        }
    }
    ...
    public static void silentCloseOutputStream(OutputStream os) {
        try {
			...
			EsLog.w(LOG_TAG, "Cannot close output stream", e);
        }
    }
    ...
    public static void endEntityViaReflection(HttpEntity entity) {
        if (entity instanceof HttpEntityWrapper) {
            try {
				...
				EsLog.e(LOG_TAG, "wrappedEntity consume", t);
            }
        }
    }
