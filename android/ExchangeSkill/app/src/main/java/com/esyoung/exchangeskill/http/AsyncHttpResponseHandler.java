...
package com.esyoung.exchangeskill.http;
...
public abstract class AsyncHttpResponseHandler implements ResponseHandlerInterface {
     ...
     */
    public void onProgress(int bytesWritten, int totalSize) {
		EsLog.d(LOG_TAG, String.format("Progress %d from %d (%2.0f%%)", bytesWritten, totalSize,
     ...
     */
    public void onRetry(int retryNo) {
		EsLog.d(LOG_TAG, String.format("Request retry no. %d", retryNo));
    }

    public void onCancel() {
		EsLog.d(LOG_TAG, "Request got cancelled");
    }
        ...
        switch (message.what) {
            case SUCCESS_MESSAGE:
                ...
                if (response != null && response.length >= 3) {
					...
					EsLog.e(LOG_TAG, "SUCCESS_MESSAGE didn't got enough params");
                }
                break;
            case FAILURE_MESSAGE:
                ...
                if (response != null && response.length >= 4) {
					...
					EsLog.e(LOG_TAG, "FAILURE_MESSAGE didn't got enough params");
                }
                break;
            case START_MESSAGE:
                onStart();
            ...
            case PROGRESS_MESSAGE:
                ...
                if (response != null && response.length >= 2) {
                    try {
						...
						EsLog.e(LOG_TAG, "custom onProgress contains an error", t);
                    }
                } else {
					EsLog.e(LOG_TAG, "PROGRESS_MESSAGE didn't got enough params");
                }
                break;
            case RETRY_MESSAGE:
                response = (Object[]) message.obj;
                if (response != null && response.length == 1) {
					...
					EsLog.e(LOG_TAG, "RETRY_MESSAGE didn't get enough params");
                }
                break;
            case CANCEL_MESSAGE:
                onCancel();
    ...
    protected void postRunnable(Runnable runnable) {
        if (runnable != null) {
            if (getUseSynchronousMode() || handler == null) {
				...
				// AssertUtils.asserts(handler != null,
